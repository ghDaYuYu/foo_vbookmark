...
bool CBookmarkPreferences::cfg_config_1_has_changed() {
	...
	result |= isUiChanged(wnd, eat_dst_rec_path);
...
INT_PTR WINAPI CBookmarkPreferences::on_config_1_dialog_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_COMMAND:
		...
		if (HIWORD(wp) == CBN_SELCHANGE) {
			...
			else if (nId == IDC_BUTTON_DST_REC) {
                pfc::string8 buffer;

                if (TRUE == uGetOpenFileName(m_hWnd, "Python (*.py)|*.py\0", OFN_PATHMUSTEXIST, "*.py",
                    "Please select Python recording script", NULL, buffer, FALSE))
                {
                    std::filesystem::path os_file = std::filesystem::u8path(buffer.c_str());

                    bool sec_level = os_file.parent_path().compare(os_file.parent_path().parent_path());
                    if (sec_level) {
                        uSetDlgItemText(m_hWnd, IDC_EDIT_REC_DST, buffer);
                        OnChanged();
                        return FALSE;
                    }
                }
